...
void discovery(){
	int recv_sock, send_sock1, send_sock2;
	struct sockaddr_in adr, broad_adr;
	FILE *fp;
	int str_len;
	int so_brd = 1;
	int port = -1;
	char buf[30], sendbuf[30];
	
	recv_sock=socket(PF_INET, SOCK_DGRAM, 0);
	memset(&adr, 0, sizeof(adr));
	adr.sin_family = AF_INET;
	adr.sin_addr.s_addr = htonl(INADDR_ANY);
	adr.sin_port = htons(atoi("8080"));

	send_sock1 = socket(PF_INET, SOCK_DGRAM, 0);
	memset(&broad_adr, 0 , sizeof(broad_adr));
	broad_adr.sin_addr.s_addr = inet_addr("255.255.255.255");
	broad_adr.sin_port = htons(atoi("8081"));
	setsockopt(send_sock1, SOL_SOCKET, SO_BROADCAST, (void*)&so_brd, sizeof(so_brd));

	send_sock2 = socket(PF_INET, SOCK_DGRAM, 0);
	memset(&broad_adr, 0 , sizeof(broad_adr));
	broad_adr.sin_addr.s_addr = inet_addr("255.255.255.255");
	broad_adr.sin_port = htons(atoi("8082"));
	setsockopt(send_sock2, SOL_SOCKET, SO_BROADCAST, (void*)&so_brd, sizeof(so_brd));

	if(bind(recv_sock, (struct sockaddr*)&adr, sizeof(adr)) == -1)
		error_handling("bind() error");
	
	while(1){
		str_len = recvfrom(recv_sock, buf, 30-1, 0, NULL, 0);
		buf[str_len] = 0;
		buf[6] = 0;
		...
		if(strcmp(buf,"server") == 0){
			...
			sendto(send_sock1, sendbuf, strlen(sendbuf), 0, (struct sockaddr*)&broad_adr, sizeof(broad_adr));
		}
			...
			sendto(send_sock2, sendbuf, strlen(sendbuf), 0, (struct sockaddr*)&broad_adr, sizeof(broad_adr));
		}
	}
}
